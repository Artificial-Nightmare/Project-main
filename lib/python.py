...
dll_path = os.path.join(current_dir, 'ClassLin.dll')
...
dll.linear_classification_export.argtypes = [ctypes.POINTER(ctypes.POINTER(ctypes.c_double)), ctypes.POINTER(ctypes.c_int), ctypes.c_double, ctypes.c_int, ctypes.c_char_p]
dll.linear_classification_export.restype = ctypes.POINTER(ctypes.c_double)
...
resultat = dll.linear_classification_export(X_array, Y_array, learning_rate, max_iterations, filename)
